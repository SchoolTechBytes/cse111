# Title: W02 Project: Password Strength
...
# Added a user friendly label that tells the user a rating

# Please make sure to verify the file locations in password_strength function to be looking fo the correct file on your system.
...
#     
# A password’s strength is calculated based on several factors including, is the password a dictionary word, is the password a known password, the length of the password and the complexity of the password. Here are the requirements:
...
def main():
    """
     	Provides the user input loop. 
        The loop asks the user for a password to test. 
    ...
    """
    while True:
        user_password = input("Please enter the password you want to check (or Q to quit): ")
        ...
        if (user_password == 'q') or (user_password == 'Q'):
            ...
            print(f"Password strength score: {strength} (0 to 5)\n")
...
def word_in_file(word, filename, case_sensitive=False):
    ...
    If the parameter case_sensitive is true a case sensitive match is performed. 
    If case_sensitive is false a case insensitive match is performed. 
...
def word_has_character(word, character_list):
    """
    This function loops through each character in the string passed in the word parameter 
    to see if that character is in the list of characters passed in the character_list parameter. 
...
def word_complexity(word):
    ...
    The function calls the word_has_character function for each of the 4 kinds of characters 
    (LOWER, UPPER, DIGITS, SPECIAL). If the word has that kind of character a point is added to complexity rating. 
...
def password_strength(password, min_length=10, strong_length=16):
    ...
    calls word_complexity to calculate the word's complexity then determines 
    the password's strength based on the user requirements. 
    It should print the messages defined in the requirements and 
    ...
    if word_in_file(password, 'wordlist.txt', case_sensitive=True):
    ...
    if len(password) < min_length:
        print("Password is too short and is no secure.")
        strength = 1
        label = strength_label(strength)
        print(f"Strength rating: {label}")
        return 1
